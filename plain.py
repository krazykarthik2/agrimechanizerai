...
spray_bits = [0] * 10 # Used for UI visualization
human_bits = [0] * 10
left_spray_bit = 0
right_spray_bit = 0
...
def draw_working_screen(img, frame_a, frame_b, safety_dets, mode_dets):
    ...
    # UI circles updated to reflect the new master bits
...
while is_running:
    ...
    if current_screen == "working":
        ...
        # 2. Target Detection based on Mode
        if selected_mode == "target":
            gray = cv2.cvtColor(panorama, cv2.COLOR_BGR2GRAY)
            edges = cv2.dilate(cv2.Canny(cv2.GaussianBlur(gray, (5, 5), 0), 50, 150), np.ones((5,5), np.uint8), iterations=1)
            for i, pt in enumerate(ALL_POINTS):
                px, py = pt[0], pt[1] - 150 
                if 0 <= py < 480:
                    roi = edges[max(0, py-20):min(480, py+20), max(0, px-20):min(1280, px+20)]
                    if cv2.countNonZero(roi) > 15: mode_bits[i] = 1
        ...
        # 3. CONSOLIDATED LOGIC (Single bit per side)
        # Check if a human is detected in any of the 5 points per side
        human_left = any(human_bits[0:5])
        human_right = any(human_bits[5:10])
        
        # Count target points per side
        targets_left = sum(mode_bits[0:5])
        targets_right = sum(mode_bits[5:10])

        # Apply Rule: Spray 0 if human present. Else, spray 1 if > 2 targets detected.
        ...
        # 4. Serial Outbound: Simplified Single-Bit Protocol
        if ser:
            ser.write(f"write:{left_spray_bit};{right_spray_bit}\r\n".encode())
